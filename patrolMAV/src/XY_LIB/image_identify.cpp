...
int use_gps_height_filter_offset(Point3f* dist, vector<Point3f> src)
{
	...
	{
		//if(fabs(1.6 - (src[i].z / 100)) <= 4.0 || 8.0 - DIFF_HEIGHT_WHEN_TAKEOFF > cur_pos.height)	//0329 (2.0 to 4.0) , add the height if below 8.0 always ok
		if(fabs(cur_pos.height - (src[i].z / 100)) <= 4.0 || 8.0 - DIFF_HEIGHT_WHEN_TAKEOFF > cur_pos.height)	//0329 (2.0 to 4.0) , add the height if below 8.0 always ok
		{
...
/*
		if(fabs(cur_pos.height - (src[i].z / 100)) > 4.0)	//0329 (2.0 to 4.0)
		{
			continue;
		}
		else
