...
static void XYI_Saturation(double *_src_data,double _limit)
{
	double data = *_src_data;
	_limit = fabs(_limit);
	if( data > _limit )
	{
		data = _limit;
...
void init_g_origin_pos(XYI_global_position_float *_g_origin_pos)
{
...
void geo2XYZ(XYI_global_position_float pos, XYZ *pXYZ)
{
...
void XYZ2xyz(XYI_global_position_float s_pos, XYZ pXYZ, Center_xyz *pxyz)
{  	
	XYZ s_XYZ, temp_XYZ;
...
int XY_Ctrl_Drone_To_Assign_Height_Has_MaxVel_And_FP_DELIVER(float _max_vel, float _min_vel, float _t_height, float _threshold, double _kp_z)
{
    ...
    XYI_global_position_float _focus_point;
    ...
    while(1)
    {
		...
		//LLL: above variable re-definition
		...
		cur_body_angle.yaw_deg = 0.95 * cur_body_angle.yaw_deg; //add 0314
...
int XY_Ctrl_Drone_P2P_With_FP_COMMON(float _p2p_height, int _goback)
{
	...
	//LLL: above variables re-definition
    ...
    while(1)
    {
		...
		//printf("oxyz.x=%.4f,oxyz.y=%.4f, ",oxyz.x,delta_y);
		...
		rT_x = oxyz.x + designatedV * periodT * rT_sin * cycle;
		rT_y = oxyz.y + designatedV * periodT * rT_cos * cycle;

		//printf("rT_sin=%f,rT_cos=%f, ",rT_sin,rT_cos);
		//printf("oxyz.x=%f,oxyz.y=%f\n",oxyz.x,oxyz.y);
		//printf("rT_x=%f,rT_y=%f, ",rT_x,rT_y);
		//printf("exyz.x=%f,exyz.y=%f, ",exyz.x,exyz.y);
		...
		if (targetError < (1 * HOVER_POINT_RANGE)) 	//Get the point and exit,0.1 is HOVER_POINT_RANGE_Critical        
		{
			...
			if (cur_legn->next != NULL) 
			{
				...
				printf("next exyz.x=%f, next exyz.y=%f\n",exyz.x,exyz.y);
			}
			else 
			{
				//printf("keep\n");
			...
			//printf("controlError=%.4f, ",controlError);
		...
		dir_Yaw = atan2((rT_y - cxyz.y),(rT_x - cxyz.x)) * 180 / PI; // aim to next route point --zzy0309  
		//printf("dir_Yaw = %.4f\n",dir_Yaw);
		
		//printf("x_n_vel=%.4f, y_e_vel=%.4f, \n",x_n_vel,y_e_vel);
...
int XY_Ctrl_Drone_Spot_Hover_And_Find_Put_Point_DELIVER(void)
{
    ...
    while(1)
    {
		...
		no_image_timeout_count++;
	   
		if ( 500 <= no_image_timeout_count)	//0330 wait to get image 10s
		{
			return 2;
			printf("[WARNGING] No Image Offset!No Image Offset!No Image Offset!No Image Offset!\n");
		}
	   
		if( XY_Get_Offset_Data(&offset, OFFSET_GET_ID_A) == 0 )
        {
			no_image_timeout_count = 0;
...
int XY_Ctrl_Drone_Down_Has_NoGPS_Mode_And_Approach_Put_Point_GOBACK(float _max_vel, float _min_vel, float _t_height, float _threshold, double _kp_z)
{
	...
	//		change_image_version("3");
		...
		if( return_timeout_flag == 1 )
		{
			...
			if(return_timeout >= (((HEIGHT_TO_USE_ULTRA-(DELIVER_HEIGHT_OF_DOWNH2+DELIVER_THRESHOLD_OF_DOWN_TO_H2_OUT))/MIN_Z_VEL_FOR_DROP+0.05)/DT))//vel use the min for drop0330
			{
				printf("Return timeout! Ready to drop!\n");
  //0330,no return!              return 1;
			}
			return_timeout++;
		}
		...
		if(XY_Get_Ultra_Data(&ultra_tmp, ULTRA_GET_ID_B) == 0)
		{
			...
			/*valid ultra data*/
			if ( 0 < ultra_height && HEIGHT_TO_USE_ULTRA > ultra_height && ( HEIGHT_TO_USE_ULTRA + 2.0 - DIFF_HEIGHT_WHEN_TAKEOFF ) > _cpos.height )// ultra_height < 10.0-->ultra_height < 4.0, _cpos.height<6.0 by juzheng 0306
			{
				...
				if (ultra_height < 3.5 && ultra_height_is_low == 0)
				{
				...
				printf("ultra_filter_use=%.4f nogps_z=%.f4\n",ultra_height, cxyz_no_gps.z);
		...
		if( XY_Get_Offset_Data(&offset, OFFSET_GET_ID_A) == 0 )
		{
			...
			if (offset.z < 3.5 && offset.z > 0 && ( 3.5 + 2.0 - DIFF_HEIGHT_WHEN_TAKEOFF ) > _cpos.height )
			{
		...
		//0329, when the ultra data can be use, make the vertical down vel to 0.20-0.25, more slowly to catch the mark
		if ( 1 == ultra_height_use_flag )
		{	
			...
			if( user_ctrl_data.thr_z > 0 )
			{
				if( user_ctrl_data.thr_z < MIN_Z_VEL_FOR_DROP)
					user_ctrl_data.thr_z = MIN_Z_VEL_FOR_DROP;
				
				if( user_ctrl_data.thr_z > MAX_Z_VEL_FOR_DROP)
					user_ctrl_data.thr_z = MAX_Z_VEL_FOR_DROP;
			}
			else
			{
				if( user_ctrl_data.thr_z > (0 - MIN_Z_VEL_FOR_DROP) )
					user_ctrl_data.thr_z = 0 - MIN_Z_VEL_FOR_DROP;
				
				if( user_ctrl_data.thr_z < (0 - MAX_Z_VEL_FOR_DROP) )
					user_ctrl_data.thr_z = 0 - MAX_Z_VEL_FOR_DROP;
			}
		}
		else
		{
